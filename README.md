...
### Minecraftサーバー自動基盤構築システム

#### プロジェクト概要

AWS上でMinecraftサーバーを効率的に運用するための完全自動化基盤を構築。
Terraformによるインフラのコード化とDiscordボットによる運用自動化により、運用負荷削減を図りつつ、VPSサービスと比較してコスト最適化を実現。

#### 技術スタック

**インフラ構成**
- **AWS EC2**: ゲームサーバーホスティング
- **AWS Lambda**: サーバーレスコンピューティング、イベント駆動処理
- **API Gateway**: RESTful API、Webhookエンドポイント
- **Amazon SNS**: イベント駆動アーキテクチャ、非同期処理

**運用・監視**
- **Discord Bot**: インタラクティブなサーバー制御インターフェース
- **CloudWatch Agents**: カスタムメトリクス収集、パフォーマンス監視
- **AWS DLM**: 自動バックアップ管理、ポイントインタイムリカバリ
- **Route 53**: 動的DNS管理、サーバー起動時の自動レコード更新

#### 実装機能・特徴

**自動化システム**
- **完全IaC化**: Terraformによる全リソースのコード管理
- **Discord統合**: チャットベースのサーバー起動・停止制御
- **DiscordボットDeferred Interaction Response対応**: 非同期処理による高速レスポンス
- **イベント駆動アーキテクチャ**: API Gateway → Lambda → SNS → Lambdaの非同期処理パイプライン

**コスト最適化**
- **自動起動・停止**: 使用時のみDiscordからカスタムコマンドでサーバー稼働によるコスト削減
- **リソース調整**: CloudWatchカスタムメトリクスに基づくリソース監視

**運用効率化**
- **自動バックアップ**: DLMによる定期スナップショット取得
- **動的DNS更新**: サーバー起動時の自動ドメインレコード更新
- **カスタムメトリクス監視**: メモリ使用率、プレイヤー数等の詳細監視
- **SSMを使用したソフトウェアの自動起動/停止**: SSM Run Commandを利用したソフトウェア自動起動および停止
